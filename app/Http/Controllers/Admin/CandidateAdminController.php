...
class CandidateAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.candidates',compact('event','candidates','levels'));
    }
    ...
    {
        $sheet=$this->baseSheet('MẪU NHẬP THÍ SINH', true);
        $sheet->fromArray(['KET002','Nguyễn Văn A','KET','Lớp 6A','03','1234'],null,'A3');
        $sheet->fromArray(['PET002','Trần Thị B','PET','Lớp 7B','04','5678'],null,'A4');
        $sheet->setCellValue('A6','Lưu ý: Cấp độ chỉ nhập KET hoặc PET. PIN tối thiểu 4 ký tự. Không đổi tên hoặc thứ tự các cột.');
        $sheet->mergeCells('A6:G6');
        return $this->xlsxDownload($sheet->getParent(),'mau-nhap-thi-sinh.xlsx');
    }
    ...
    public function importExcel(Request $request)
    {
        ...
        $levels=Level::all()->keyBy(fn($l)=>strtoupper($l->code));$imported=0;$errors=[];
